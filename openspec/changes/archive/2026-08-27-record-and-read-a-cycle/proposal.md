...
- The boundary day and time zone used to resolve the active cycle are
  seeded configuration (`boundary_day = 30`, `timezone =
  'America/Bogota'`), read at setup time.
...
- **Any breakdown, chart, or trend.** "Understand my habits" needs
  ...
  cycles of use.
- Editing the cycle's boundary day or time zone. The seeded row is
  read-only in this change; changing it is a manual `UPDATE` against
  Neon, not a feature.
